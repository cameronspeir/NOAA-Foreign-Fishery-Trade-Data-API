---
title: Very Basic Examples of How to Interact with the NOAA Foreign Trade Data Using
  an API
...
The url <https://www.st.nmfs.noaa.gov/ords/foss/trade_data> is the default for accessing the data.  Click on the link and you'll land on a page with the first 25 rows of data in the data set.  It's not clear to me how this is sorted, but there you go.  You can get this data in R by using httr:GET()

```{r}
ex_1 <- GET("https://www.st.nmfs.noaa.gov/ords/foss/trade_data")
ex_1
http_status(ex_1)
...
We now have a list of 6 elements.  It's kind of messy to print out, but use the str() statement that is commented out above to see for yourself what this looks like.  Visual inspection of this list reveals that "items" is the element containing the actual data. We can make this element a data frame to work with.
...
Note that one of the variables in this data frame, "links", is a weird variable that contains a list in each row.  Let's get rid of that.  Otherwise, it's messy to view. You can see for yourself by running the str() and head() statements that are commented out above.
...
## Filtering the data
The real usefulness of the API with these data is filtering to get a very specific subset.  Here is an example of the URL you would use to get all data from 2011 onwards: <br>
https://www.st.nmfs.noaa.gov/ords/foss/trade_data/?q={"year":{"\$gt": 2010}}
<br>
If you copy and paste that link into a brower, it will return that specific data. 
<br>
The grammer for this type of filter is pretty specific and wasn't familar to me you.  For example, "\$gt" means "greater than".  For a list of filter operators you can use, checkout "3.2.5.2.1 FilterObject Grammar" at this link: <https://docs.oracle.com/cd/E56351_01/doc.30/e87809/developing-REST-applications.htm#AELIG90103>
<br>
...
<br>
I found the httr:GET() syntax a little tricky.  The query argument consists of a list with *one* element. The element is named "q".  Put all of the filter objects, including the brackets, between single quotes. 

```{r}

ex_2 <- GET("https://www.st.nmfs.noaa.gov/ords/foss/trade_data",
  query = list(q = '{"name":{"$like":"%SALMON%"},"source":"EXP","district_name":"SEATTLE, WA","year":"2016"}'))

ex_2

ex_2_list <- jsonlite::fromJSON(rawToChar(ex_2$content))

ex_2_df <- ex_2_list$items
